...
| **Pagination** | `Covered` | **100%** | <ul><li>[page.tsx](file:///Users/bits-blr-sangiliboopathi/HRMS_NodeJs/frontend/src/pages/HRM-Config/region/page.tsx)</li><li>[table-footer.tsx](file:///Users/bits-blr-sangiliboopathi/HRMS_NodeJs/frontend/src/components/table/table-footer.tsx)</li></ul> | <ul><li>[test_08_pagination.py](file:///Users/bits-blr-sangiliboopathi/HRM_GENIE_TEST_SCRIPTS/tests/HRM_Configuration/Region/test_08_pagination.py)</li></ul> | End-to-end pagination test: seeds the table to ≥ 21 records if needed, verifies `Showing 1 to 10 of N entries` on **Page 1**, navigates to **Page 2** (`Showing 11 to N of N entries`), uses **Go to previous page** and **Go to next page** aria-labelled controls via `onPageChange`, verifies `aria-current="page"` attribute on active page buttons. Also explicitly tests **page size switching** via `onEntriesPerPageChange`: selects 20 → 10 per page via Radix UI select, asserting `Showing 1 to 20 of N entries` on page 1, `Showing 21 to N of N entries` on page 2 at 20/page, and page 2 reappearing on revert to 10/page. Cleans up test data afterwards. |
...
* **Page Size Selection (`onEntriesPerPageChange`)** — explicitly tested in `test_08_pagination.py` (Step 8): switches 10 → 20 → 10 per page, asserting `Showing 1 to X of N` text adapts correctly for each size and page 2 reappears on revert to 10.
...
8. [test_08_pagination.py](file:///Users/bits-blr-sangiliboopathi/HRM_GENIE_TEST_SCRIPTS/tests/HRM_Configuration/Region/test_08_pagination.py) — Pagination & Page Size Selection (page 1/2 navigation, prev/next, 10/20 entries per page switching)
