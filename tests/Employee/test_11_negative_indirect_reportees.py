"""
Negative test cases for Employee module: Indirect Reportees.

Covers:
  - Indirect Reportees search/filter degrade gracefully on invalid input (no crash,
...
LIST_PATH = "/employee/indirect-report"
...
def _navigate_to_list(page: Page):
    """Navigates to indirect reportees tab."""
    page.goto(f"{BASE_URL}{LIST_PATH}")
    page.wait_for_timeout(1000)
    btn = page.get_by_role("button", name="Indirect Reportees")
...
# ---------------------------------------------------------------------------
# Indirect Reportees - search & filter negative cases
# ---------------------------------------------------------------------------

@pytest.mark.parametrize("garbage_query", GARBAGE_QUERIES)
def test_indirect_reportees_search_invalid_query_shows_no_results(
    ...
    try:
        ...
        _screenshot(page, "test_11_no_results_indirect_reportees")
...
def test_indirect_reportees_search_script_injection_is_neutralized(
    ...
    try:
        ...
        _screenshot(page, "test_11_xss_indirect_reportees")
...
def test_indirect_reportees_filter_mismatched_combination_returns_empty_gracefully(
    playwright: Playwright
) -> None:
    browser, context, page = create_page(playwright)
    try:
        ...
        _screenshot(page, "test_11_filter_mismatch_indirect_reportees")
...
def test_indirect_reportees_detail_view_from_list_is_read_only(
    playwright: Playwright
) -> None:
    browser, context, page = create_page(playwright)
    try:
        ...
        if employee_row_cell.count() > 0:
            ...
            page.wait_for_timeout(1000)
            _screenshot(page, "test_11_detail_readonly_indirect_reportees")
            assert_page_is_read_only(page)
        else:
            pytest.skip("No employee rows available to open from indirect reportees")
    finally:
        context.close()
        browser.close()
...
if __name__ == "__main__":
    with sync_playwright() as playwright:
        for query in GARBAGE_QUERIES:
            test_indirect_reportees_search_invalid_query_shows_no_results(playwright, query)
        test_indirect_reportees_search_script_injection_is_neutralized(playwright)
        test_indirect_reportees_filter_mismatched_combination_returns_empty_gracefully(playwright)
        test_indirect_reportees_detail_view_from_list_is_read_only(playwright)
