...
def test_combined_search(playwright: Playwright) -> None:
    ...
    reset_filters(page)
    _screenshot(page, "test_05_before_combined_search")
    ...
    _screenshot(page, "test_05_branch_then_text")
    ...
    page.get_by_role("textbox", name="Search", exact=True).press("Enter")
    page.wait_for_timeout(500)
    search_and_select_branch(page, "Mumb", "Mumbai, Kolkata, Pune")
    expect(
        page.get_by_role("combobox").filter(has_text="Mumbai, Kolkata, Pune")
    ).to_be_visible()
    page.wait_for_timeout(1000)
    print("Text + branch search applied")
    _screenshot(page, "test_05_text_then_branch")
    ...
    search_and_select_branch(page, "Tumkur", "Kerala, Kozhikode, Tumkur")
    search_text(page, "NonExistentRegion123")
    page.get_by_role("textbox", name="Search", exact=True).press("Enter")
    page.wait_for_timeout(1000)
    print("Non-matching combined filter applied")
    _screenshot(page, "test_05_no_match_combined")
    ...
    _screenshot(page, "test_05_reset_cleared")

    # --- 5. Branch select, then clear text, re-search text ---
    search_and_select_branch(page, "Mumbai", "Mumbai, Kolkata, Pune")
    expect(
        page.get_by_role("combobox").filter(has_text="Mumbai, Kolkata, Pune")
    ).to_be_visible()
    search_text(page, "South West")
    ...
    _screenshot(page, "test_05_branch_persisted_text_changed")
